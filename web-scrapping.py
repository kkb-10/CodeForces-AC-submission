...
no_of_pages=12                # No of submission pages in your profile
path="D:\code\Code-Library\ "  # Path where you want to save all AC submssion in your local repo
partymemberid="9214927"        # Inspect element any submission id from your profile and get your partymemberid from there
...
for i in range (start_page,no_of_pages+1):
    ...
    for row in table:
        ...
        if(subm_verdict=="Accepted"):   # if we get an AC submission 
            ...
            final_path = path + "problem" + str(count) + " - code.txt"
            
            req_path = path + problem_name + "." + subm_type
            if not os.path.exists(path + problem_name + "." + subm_type): # If that file name already exists, then it doesn't add it to repo
            ...
            print(problem_name)
            print("Saving file at: " + path + "problem" + str(count) + " - code." + subm_type ,end="\n\n")
            count+=1
